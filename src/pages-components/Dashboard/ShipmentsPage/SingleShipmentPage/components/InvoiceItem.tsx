...
export const InvoiceItem: React.FC<InvoiceItemProps> = ({
  ...
  return (
    <FlexLayout className="flex-col gap-1 relative">
      ...
      {invoiceStatusUpdatedAt && (
        <Text className="self-end absolute bottom-[-28px] right-0" color="text-color-4" variant="text-xxs">
          Status ažuriran: {getDateTimeInLocalTimezone(invoiceStatusUpdatedAt)}
        </Text>
      )}
    </FlexLayout>
  );
