...
export const Radio: React.FC<RadioProps> = ({ isDisabled, option }) => {
  return (
    ...
    >
      <HeadlessRadio value={option.value} className="group flex items-center gap-3 p-4">
        <Box
          className={clsx(
        ...
        <Box>
          <Label>
            <Text color="text-color-1" className={clsx(!isDisabled && 'cursor-pointer')} variant="text-s">
              {option.label}
            </Text>
          </Label>
        </Box>
